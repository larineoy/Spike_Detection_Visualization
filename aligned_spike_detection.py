...
output_dir = "/Users/larineouyang/aligned_spike_detection_result"
...
def process_and_plot(spike_prob_file, spectrogram_file, output_file):
    ...
    # Load the spectrogram image and its size
    spectrogram = Image.open(spectrogram_file)
    spectrogram_array = np.array(spectrogram)
    
    # Generate the corresponding time axis for the spectrogram
    spectrogram_duration = len(time)
    extent = [0, spectrogram_duration, 0, spectrogram_array.shape[0]]

    # Plot the data with two vertically aligned subplots
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 8), sharex=True)

    # Display the spectrogram
    ax1.imshow(spectrogram_array, aspect='auto', extent=extent)
    ...
    # Ensure that both plots have the same x-axis (time) limits
    ax2.set_xlim(0, spectrogram_duration)
